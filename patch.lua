...
function repr(str)
    result,_ = string.format("%q", str):gsub("\\\n", "\\n")
    return result
end
...
function BasePatchInstruction:new (o,url,further_patching_func,biggest_possible_size,find_string_start,respect_https,base_patch_group)
...
function BasePatchInstruction:print()
	local print_result = string.format("BasePatchInstruction:new(%s, %s, %d, %s, %s, %d); found_a_match = %s", repr(self.url), tostring(self.further_patching_func), self.biggest_possible_size, repr(self.find_string_start), tostring(self.respect_https), self.base_patch_group, tostring(self.found_a_match))
	print(print_result)
end
...
function lbp_main_is_offset_valid(offset, file, url, respect_https)
	...
	local existing_url_orig,in_size_url = check_if_is_valid_null_termed(offset, file, url, BIGGEST_POSSIBLE_URL_IN_EBOOT_INCL_NULL)
	if not in_size_url then
		return false
	end
...
function basic_replace(offset, file, thing,biggest_possible_size,thing_type_str_for_error)
	...
	if string.len(thing) > in_size_url then
		error("Found a valid "..thing_type_str_for_error.." "..existing_url_str.." but input "..thing_type_str_for_error.." is too big")
	end
...
list_of_patches)
	...
	local is_function_succeed,result = pcall(function()
		...
		for _,value in ipairs(list_of_patches) do
			if value.biggest_possible_size > trailing_buffer_size then trailing_buffer_size = value.biggest_possible_size end
		end
		while true do
				...
				if done_groups_dict[patch_to_do.base_patch_group] and done_groups_dict[patch_to_do.base_patch_group] ~= index_patch_to_do then
					...
					for offset in gen_maker_two_args(find_all_in_str,chunk,patch_to_do.find_string_start) do
						...
						if patch_to_do.further_patching_func(offset,file,patch_to_do.url,patch_to_do.respect_https) then
							for inner_patch_to_do, inner_patch_to_do in ipairs(list_of_patches) do
...
function internal_patch_lbp_main(eboot_elf_path, url, digest, normalise_digest, working_dir, respect_https, lbp_vita_loose_domain_for_cross_controller_app)
	---@type (BasePatchInstruction)[]
	local patches_list = {}
	local base_patch_instruction = BasePatchInstruction:new(nil,url.."\x00",lbp_main_is_offset_valid,BIGGEST_POSSIBLE_URL_IN_EBOOT_INCL_NULL,"http",respect_https,1)
	...
	if digest ~= '' then
		for _, digest_to_find in ipairs(REPLACE_DIGESTS) do
			base_patch_instruction = BasePatchInstruction:new(nil,digest.."\x00",lbp_main_digest_patch,BIGGEST_POSSIBLE_DIGEST_IN_EBOOT_INCL_NULL,digest_to_find,false,2)
			table.insert(patches_list,base_patch_instruction)
		end
	end
	if lbp_vita_loose_domain_for_cross_controller_app then
		url = extract_domain_from_url(url)
		if not url then
			error("url does not start with http:// or https://")
		end
		
		base_patch_instruction = BasePatchInstruction:new(nil,url.."\x00",lbp_vita_loose_domain_for_cross_controller_app_patch,BIGGEST_POSSIBLE_URL_IN_EBOOT_INCL_NULL,LBP_VITA_URL_FOR_CROSS_CONTROLLER,false,3)
		table.insert(patches_list,base_patch_instruction)
	end
	...
	for _,value in ipairs(patches_list) do
		...
		if value.base_patch_group == 3 and not value.found_a_match then
			error("Could not lbp vita cross controller domain, likely you gave an eboot that is not the cross controller app")
		end
	end
	return true
end
...
function patch_ps3_lbpk(eboot_elf_path, url, digest, normalise_digest, working_dir)
	---@type (BasePatchInstruction)[]
	local patches_list = {}
	local base_patch_instruction = BasePatchInstruction:new(nil,url.."\x00",lbpk_patch,BIGGEST_POSSIBLE_URL_IN_LBPK_EBOOT_INCL_NULL,"lbpk.ps3.online.scea.com\x00",false,1)
	table.insert(patches_list,base_patch_instruction)
	base_patch(eboot_elf_path,working_dir,patches_list)
