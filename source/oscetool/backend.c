...
static int set_encrypt_options_from_template(encrypt_options_t *opts) {
  ...
  if (!template) {
    dbglogger_log("[*] Error: Could not load template %s\n", template_path);
    return 0;
  }
  
  sce_info_t *sce_info = process_sce_file(template);
  if (!sce_info) {
    dbglogger_log("[*] Error: Could not process template %s\n", template_path);
    free(template);
    return 0;
  }
  
  if (!decrypt_header(sce_info, NULL, NULL)) {
    dbglogger_log("[*] Warning: Could not decrypt template header.\n");
    free(sce_info);
    free(template);
    return 0;
  }
  ...
  if (verbose)
    dbglogger_log("[*] Template header decrypted.\n");
...
static patch_options_t *set_patch_options() {
  ...
  if (sys_process_sdk_version_value) {
	if (strlen(sys_process_sdk_version_value) != 8) {
    		dbglogger_log("[*] Warning: SDK Version should be 4 bytes long.\n");
		return NULL;
	}
...
int set_encrypt_options(encrypt_options_t *opts) {
  if (!key_revision_value) {
    dbglogger_log("[*] Error: Please specify a key revision.\n");
    return 0;
  }

  if (!valid_hex(key_revision_value)) {
    dbglogger_log("[*] Error (Key Revision): Please provide a valid hexadecimal number.\n");
    return 0;
  }
  opts->key_revision = x_to_u64(key_revision_value);
  if (!auth_id_value) {
    dbglogger_log("[*] Error: Please specify an auth ID.\n");
    return 0;
  }

  opts->auth_id = x_to_u64((char *)auth_id_value);
  if (!vendor_id_value) {
    dbglogger_log("[*] Error: Please specify a vendor ID.\n");
    return 0;
  }

  opts->vendor_id = x_to_u64((char *)vendor_id_value);
  if (!selftype_value) {
    dbglogger_log("[*] Error: Please specify a SELF type.\n");
    return 0;
  }

  uint64_t self_type = name2id(selftype_value, self_short_name_types, 0xFFFFFFFF);
  if (self_type == 0xFFFFFFFF ) {
    dbglogger_log("[*] Error: Invalid SELF type.\n");
    return 0;
  }
  opts->self_type = self_type;

  if ( !app_version_value) {
    dbglogger_log("[*] Error: Please specify an application version.\n");
    return 0;
  }
  ...
  if (self_control_flags_value) {
    if (strlen(self_control_flags_value) != 64) {
      dbglogger_log("[*] Error: Control flags need to be 32 bytes.\n");
      return 0;
    }
  ...
  if (self_capability_flags_value) {
    if (strlen(self_capability_flags_value) != 64) {
      dbglogger_log("[*] Error: Capability flags need to be 32 bytes.\n");
      return 0;
    }
...
int set_npdrm_encrypt_options(encrypt_options_t *opts) {
  ...
  if (!np_license_type_value) {
    dbglogger_log("[*] Error: Please specify a license type.\n");
    return 0;
  }
  if ( strcmp(np_license_type_value, "FREE") ) {
    if ( strcmp(np_license_type_value, "LOCAL") ) {
      dbglogger_log("[*] Error: Only supporting LOCAL and FREE license for now.\n");
      return 0;
    }
  ...
  if (!np_app_type_value) {
    dbglogger_log("[*] Error: Please specify an application type.\n");
    return 0;
  }
  
  uint64_t app_type = name2id(np_app_type_value, application_types, 0xFFFFFFFF);
  if ( app_type == 0xFFFFFFFF ) {
    dbglogger_log("[*] Error: Invalid application type.\n");
    return 0;
  }
  npdrm_info->app_type = app_type;
  if (!np_content_id_value) {
    dbglogger_log("[*] Error: Please specify a content ID.\n");
    return 0;
  }
  ...
  if (!np_realfilename_value) {
    dbglogger_log("[*] Error: Please specify a real filename.\n");
    return 0;
  }
...
int backend_cmd_print() {
  ...
  if (!inputfile) {
    dbglogger_log("[*] Error: Could not load %s\n", inputfile_path);
    return 0;
  }

  sce_info_t *sce_info = process_sce_file(inputfile);
  if (!sce_info) {
    dbglogger_log("[*] Error: Could not process %s\n", inputfile_path);
    free(inputfile);
    return 0;
  }
  ...
  if (meta_info_value) {
    if ( strlen(meta_info_value) != 128 ) {
      dbglogger_log("[*] Error: Metadata info needs to be 64 bytes.\n");
      return 0;
    }
  ...
  if (keyset_value) {
    if ( strlen(keyset_value) != 220 ) {
      dbglogger_log("[*] Error: Keyset has a wrong length.\n");
      return 0;
    }
  ...
  if (decrypt_header(sce_info, keyset_override, metadata_override)) {
    if (verbose)
      dbglogger_log("[*] Header decrypted.\n");

    if (decrypt_sections(sce_info)) {
      if ( verbose != 1 )
	dbglogger_log("[*] Data decrypted.\n");

    } else {
      dbglogger_log("[*] Warning: Could not decrypt data.\n");
    }
  }else {
    dbglogger_log("[*] Warning: Could not decrypt header.\n");
  }
...
void backend_cmd_decrypt() {
  ...
  if (sce_info->sce_header->type == SCE_TYPE_SELF) {
    if (write_elf(outputfile_path, sce_info)) {
      dbglogger_log("[*] ELF written to %s.\n", outputfile_path);
    } else {
      dbglogger_log("[*] Error: Could not write ELF.\n");
    }
  } else if (sce_info->sce_header->type == SCE_TYPE_RVK) {
    if (write_rvk(outputfile_path, sce_info)) {
      dbglogger_log("[*] RVK written to %s.\n", outputfile_path);
    } else {
      dbglogger_log("[*] Error: Could not write RVK.\n");
    }
    ...
    if (write_spp(outputfile_path, sce_info)) {
      dbglogger_log("[*] SPP written to %s.\n", outputfile_path);
    } else {
      dbglogger_log("[*] Error: Could not write SPP.\n");
    }
  }
  free(inputfile);
  free(sce_info);	
...
void backend_cmd_encrypt() {
  ...
  if (!filetype_value) {
    dbglogger_log("[*] Error: Please specify a file type.\n");
    return;
  }
  keyset_raw_t *keyset_override = NULL;
  if (keyset_value) {
  ...
  if (!inputfile) {
    dbglogger_log("[*] Error: Could not read %s\n", inputfile_path);
    return;
  }
  ...
  if ( strcmp(filetype_value, "SELF") ) {
    ...
    if (!build_self(sce_info, &opts)) {
      dbglogger_log("[*] Error: SELF not built.\n");
      return;
    }
    dbglogger_log("[*] SELF built.\n");
  ...
  if (compress_value && strcmp(compress_value, "TRUE") == 0) {
    if (can_compress) {
      ...
      dbglogger_log("[*] Data compressed.\n");
    } else {
      dbglogger_log("[*] Warning: This type of file will not be compressed.\n");
    }
  }
  ...
  if (!encrypt_metadata(sce_info, keyset_override) ) {
    dbglogger_log("[*] Error: Data not encrypted.\n");
    return;
  }

  self_encrypt_sections(sce_info);
  dbglogger_log("[*] Data encrypted.\n");
  if (!write_self(outputfile_path, sce_info)) {
    dbglogger_log("[*] Error: %s not written.\n", outputfile_path);
    return;
  }
  dbglogger_log("[*] %s written.\n", outputfile_path);
  
  if (opts.self_type == SELF_TYPE_NPDRM) {
    if (np_add_sig_value && !strcmp(np_add_sig_value, "TRUE")) {
      if (add_npdrm_footer_sig(outputfile_path))
	dbglogger_log("[*] Added NPDRM footer signature.\n");
      else
	dbglogger_log("[*] Error: Could not add NPDRM footer signature.\n");
    }
  }
}
