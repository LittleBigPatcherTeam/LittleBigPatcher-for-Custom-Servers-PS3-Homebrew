...
void decompress(uint8_t *in, uint64_t in_len, uint8_t *out, uint64_t out_len) {
  ...
  ret = inflateInit(&s);
  if (ret != Z_OK) {
    dbglogger_log("inflateInit returned %d\n", ret);
  ...
  if (ret != Z_OK && ret != Z_STREAM_END) {
    dbglogger_log("inflate returned %d\n", ret);
	second_thread_args.has_finished = 1;
    sysThreadExit(1);
  }
...
uint8_t *_read_rap_buffer(const char *file, uint32_t *length) 
{
  FILE *fp;
  uint32_t size;
  ...
  if((fp = fopen(file, "rb")) == NULL) {
	...
	if (size >= 16) { // raps can sometimes have extra data at the end, but we only care about the first 16 bytes
		size = 16;
	}
