...
#include <unistd.h> // this is for finding raps
#include <dirent.h>
#include <stdio.h>
...
static void add_key_to_keyset(const char *name, const char *value, keyset_t *keyset) {
  if (strcmp(name, "type") == 0) {
    if ( strcmp(value, "SELF") == 0) {
      ...
      dbglogger_log("[*] Error: Unknown type '%s'.\n", value);
    }
    ...
    if (strcmp(value, "LV0") == 0) {
      ...
      dbglogger_log("[*] Error: unknown SELF type '%s'.\n", value);
    }  
    ...
    dbglogger_log("[*] Error: Unknown keyfile property '%s'.\n", name);
  }
}
...
keyset_t *find_keyset_from_header(sce_info_t *sce_info) {
  ...
  if (!keyset) {
    ...
    dbglogger_log("[*] Error: Could not find keyset for %s.\n", name);
  }
  return keyset;
}
...
keyset_t *find_keyset_by_name(const char *name) {
  ...
  dbglogger_log("[*] Error: Could not find keyset '%s'.\n", name);
  return NULL;
}
...
static actdat_t *load_actdat(const uint8_t *title_id) {
  char filename[MAX_PATH] = {0};
  char tmpfilename[MAX_PATH] = {0};
  uint32_t size;

  DIR *home_dir = opendir("/dev_hdd0/home");
  struct dirent* reader;
  if (home_dir != NULL) {
  while ((reader = readdir(home_dir)) != NULL) {
		if (strcmp(reader->d_name,".") == 0 || strcmp(reader->d_name,"..") == 0) {
			continue;
		}
  sprintf(tmpfilename, "/dev_hdd0/home/%s/exdata/%s%s",reader->d_name,title_id, SCE_DATA_RIFEXT);
  
  if (does_file_exist(tmpfilename)) {
	sprintf(filename,"/dev_hdd0/home/%s/exdata/act.dat",reader->d_name);
	break;
  }
  }closedir(home_dir);}
...
static rif_t *load_rif(const uint8_t *title_id) {
  ...
  rif_t *rif = (rif_t *) _read_buffer(tmpfilename, &size);
  if (!rif)
    return NULL;
...
static uint8_t *load_rap(const uint8_t *content_id) {
  ...
  sprintf(tmpfilename, "/dev_hdd0/exdata/%s%s", content_id, SCE_DATA_RAPEXT); 
  if (does_file_exist(tmpfilename)) {
	goto found_a_rap;
  }
  DIR *home_dir = opendir("/dev_hdd0/home");
  ...
  sprintf(tmpfilename, "/dev_hdd0/home/%s/exdata/%s%s",reader->d_name,content_id, SCE_DATA_RAPEXT);
  ...
  sprintf(tmpfilename, "/dev_usb000/exdata/%s%s", content_id, SCE_DATA_RAPEXT); 
  if (does_file_exist(tmpfilename)) {
	goto found_a_rap;
  }
  sprintf(tmpfilename, "/dev_usb001/exdata/%s%s", content_id, SCE_DATA_RAPEXT); 
  if (does_file_exist(tmpfilename)) {
	goto found_a_rap;
  }
  sprintf(tmpfilename, "/dev_usb002/exdata/%s%s", content_id, SCE_DATA_RAPEXT); 
  if (does_file_exist(tmpfilename)) {
	goto found_a_rap;
  }
  sprintf(tmpfilename, "/dev_usb003/exdata/%s%s", content_id, SCE_DATA_RAPEXT); 
  if (does_file_exist(tmpfilename)) {
	goto found_a_rap;
  }
  sprintf(tmpfilename, "/dev_usb004/exdata/%s%s", content_id, SCE_DATA_RAPEXT); 
  if (does_file_exist(tmpfilename)) {
	goto found_a_rap;
  }
  sprintf(tmpfilename, "/dev_usb005/exdata/%s%s", content_id, SCE_DATA_RAPEXT); 
  if (does_file_exist(tmpfilename)) {
	goto found_a_rap;
  }
...
int decrypt_klicensee(const uint8_t *title_id, uint8_t *klic) {
  ...
  if (rap_to_klicensee(title_id, klic)) {
    if (verbose == 1)
      dbglogger_log("[*] klicensee converted from %s.rap.\n", title_id);
    return 1;
  }

  uint8_t *idps_key = (uint8_t *)second_thread_args.idps;
  if (!idps_key) {
    dbglogger_log("[*] Error: Could not load IDPS.\n");
    return 0;
  }
    
  if ( verbose == 1 )
    dbglogger_log("[*] IDPS loaded.\n");

  actdat_t *actdat = load_actdat(title_id);
  if ( !actdat ) {
    dbglogger_log("[*] Error: Could not load act.dat.\n");
    return 0;
  }

  if ( verbose == 1 )
    dbglogger_log("[*] act.dat loaded.\n");
  ...
  if (!rif) {
    dbglogger_log("[*] Error: Could not obtain klicensee for '%s'.\n", title_id);
    return 0;
  }
  ...
  if (actdat_idx > 127) {
    dbglogger_log("[*] Error: act.dat key index out of bounds.\n");
    return 0;
  }
  ...
  free(rif);
  if ( verbose == 1 )
    dbglogger_log("[*] klicensee decrypted.\n");
  return 1;
}
